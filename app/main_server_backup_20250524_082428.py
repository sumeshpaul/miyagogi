...
async def ask_hermes(
    ...
    # 🧠 MEMORY-AWARE FUZZY PRODUCT MATCHING (IMPROVED)
    try:
        recent_product_text = ""
    ...
    # Compose prompt
    prompt = (
        "You are a helpful beauty and skincare expert. Answer user queries conversationally and informatively."
        + ("\nProduct Info: " + db_context if db_context else "")
    )
    for msg in messages:
    ...
    try:
        ...
        # Format db_context for Telegram-safe HTML
        if db_context:
            db_context = db_context.replace("\n", "\n")  # Keep newlines
