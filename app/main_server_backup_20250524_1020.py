...
# ──────────────────────────────── /ask Endpoint for Hermes Chat (Fixed Final Version) ─────────────────────────────
...
async def ask_hermes(
    ...
    try:
        ...
        # Check brand-focused match first
        for brand in ["thuya", "noemi", "fairy", "lash"]:
            if brand in query_text:
                ...
                if rows:
                    db_context += f"Here are some popular {matched_brand.capitalize()} products:\n"
                    ...
                    matched_name = rows[0][0].lower()
                break

        # Fallback to best fuzzy product match
        if not matched_brand:
            cursor.execute("SELECT DISTINCT LOWER(name) FROM products")
            product_names = [row[0] for row in cursor.fetchall()]
            recent_text = user_last_product_match[user_id] if user_last_product_match[user_id] else query_text
            best_score, best_name = 0, ""
            for pname in product_names:
                score = difflib.SequenceMatcher(None, recent_text, pname).ratio()
                if score > best_score:
                    best_score, best_name = score, pname
            if best_score > 0.5:
                matched_name = best_name
                user_last_product_match[user_id] = matched_name
                cursor.execute("""
                    SELECT name, price, stock_status FROM products 
                    WHERE LOWER(name) LIKE ?
    ...
    try:
        ...
        final_response += "\n\nLet me know if you’d like recommendations or have any preferences 😊"
