...
async def ask_hermes(
    ...
    try:
        # Step 1: Determine base query text for matching
        recent_product_text = ""
        ...
        # Step 2: Check if query is brand-focused
        brand_match = None
        if any(brand in query_text for brand in ["thuya", "noemi", "fairy", "lash"]):
            brand_match = next((b for b in ["thuya", "noemi", "fairy", "lash"] if b in query_text), None)
        ...
        if brand_match:
            ...
            if rows:
                db_context = "Here are some popular {} products:\n".format(brand_match.capitalize())
